...
**Versi:** 2.0.0  
**Base URL:** `https://web-production-24bee.up.railway.app`  
**Python:** 3.10.9 | **Framework:** FastAPI 0.104.1  
**Dokumentasi interaktif:** `https://web-production-24bee.up.railway.app/docs`
...
```bash
curl https://web-production-24bee.up.railway.app/health
```
...
const response = await fetch(
  'https://web-production-24bee.up.railway.app/acne/analyze?jenis_kulit=Berminyak&top_n=5',
  {
    method: 'POST',
    body: formData,
  }
...
with open('foto_wajah.jpg', 'rb') as f:
    response = requests.post(
        'https://web-production-24bee.up.railway.app/acne/analyze',
...
```bash
curl -X POST "https://web-production-24bee.up.railway.app/acne/analyze?jenis_kulit=Berminyak&top_n=5" \
...
const response = await fetch(
  'https://web-production-24bee.up.railway.app/acne/analyze/batch?jenis_kulit=Kering&top_n=3',
...
```bash
curl -X POST "https://web-production-24bee.up.railway.app/acne/analyze/batch?jenis_kulit=Berminyak&top_n=3" \
     -F "files=@foto1.jpg" \
     -F "files=@foto2.jpg"
```
...
```bash
curl https://web-production-24bee.up.railway.app/acne/info
```
...
const response = await fetch('https://web-production-24bee.up.railway.app/crowd/predict', {
  method: 'POST',
  body: formData
});
```
...
```bash
curl -X POST https://web-production-24bee.up.railway.app/crowd/predict/batch \
     -F "files=@foto1.jpg" \
     -F "files=@foto2.jpg"
```
...
```javascript
const response = await fetch('https://web-production-24bee.up.railway.app/skincare/recommend', {
  method: 'POST',
...
```bash
curl https://web-production-24bee.up.railway.app/skincare/skin-types
```
...
async function analyzeAcne() {
  ...
  try {
    const response = await fetch(
      `https://web-production-24bee.up.railway.app/acne/analyze?jenis_kulit=${jenisSkin}&top_n=5`,
      { method: 'POST', body: formData }
    );
